...
type RangedValueOperator = "$gt" | "$gte" | "$lt" | "$lte" | "$eq" | "$neq" | "$between";

/* Loki Comparator interface for dependency injection to ranged indexes */
interface IRangedComparer<T> {
   compare(val: T, val2: T): -1 | 0 | 1;
}

interface IRangedIndexRequest<T> {
   op: RangedValueOperator;
   val: T;
   high?: T;
}

interface IComparatorMap {
   [name: string]: IRangedComparer<any>;
}

interface IRangedIndexFactoryMap {
 ...
 * @hidden
 */
function ltHelper(prop1: any, prop2: any, equal: boolean): boolean {
 ...
 * @returns {boolean}
 */
function gtHelper(prop1: any, prop2: any, equal: boolean): boolean {
   // 'falsy' and Boolean handling
   if (!prop1 || !prop2 || prop1 === true || prop2 === true || prop1 !== prop1 || prop2 !== prop2) {
      let t1: number;
      let t2: number;
